const artistEndpoint = "http://127.0.0.1:8000/api/artists";
const songsEndpoint = "http://127.0.0.1:8000/api/songs";
const albumEndpoint = "http://127.0.0.1:8000/api/albums";
...
const displayArtistProfile = async () => {
  ...
  if (matchedArtist && macthedSong) {
    ...
    artistProfile.innerHTML = `
        ...
        <div>
            <p>Artist</p>
            <a href="/artists/${matchedArtist.id}" class="font-bold hover:underline"> 
            ${matchedArtist.name} 
            </a>
        </div>    
        `;
    artistProfile.addEventListener("click", () => {
      window.location.href = `/artists/${matchedArtist.id}`;
    });
